---
title: Python으로 푸는 프로그래머스 level_2. 폰켓몬
...
[프로그래머스에서 풀기](https://programmers.co.kr/learn/courses/30/lessons/1845)
...
def solution(nums):
    number_of_target=len(nums)/2
    answer=0
    prev=set()
    for n in nums:
        if n not in prev and answer<number_of_target:
            answer+=1
            prev.add(n)
...
- **가져가도 되는 폰켓몬의 수 보다 작으면서** **뽑은 폰켓몬 집합** `prev` **에 없는 종류일 경우에만 카운팅하고** `prev` **에 담는다.**
